...
# x1, y1, x2, y2, x3, y3
modules_coordinates = [(0, 0.25, -0.2, 0, 0.2, 0), (-0.2, 0, -0.2, 0.25, 0.2, 0.25), (0, 0.25, -0.2, 0, 0.2, 0), (0, 0.24, -0.2, 0.25, 0.2, 0.25)]

# Step 1: Generate random coordinates for p4 (x4, y4)
unknown_points = []
for i in range(points_generated):
    unknown_points.append((random.uniform(-20, 20), random.uniform(-20, 20)))
...
for x1, y1, x2, y2, x3, y3 in modules_coordinates:
    ...
    for x4_ideal, y4_ideal in unknown_points:
        ...
        distance_p1_p4 = distance_p1_p4_ideal + random.uniform(0, 0.05)
        distance_p2_p4 = distance_p2_p4_ideal + random.uniform(0, 0.05)
        distance_p3_p4 = distance_p3_p4_ideal + random.uniform(0, 0.05)

        # Step 5: Use trilateration method to calculate 4th point
        coordinates, distance, angle_degrees = find_point_least_squares((x1, y1), (x2, y2), (x3, y3), distance_p1_p4, distance_p2_p4, distance_p3_p4)

        # Step 6: Compare results
        x4, y4 = coordinates
        distance_error += abs(distance_ideal - distance)
        angle_error += abs(angle_degrees_ideal - angle_degrees)

    print("Average error")
    print(distance_error/points_generated, angle_error/points_generated)
